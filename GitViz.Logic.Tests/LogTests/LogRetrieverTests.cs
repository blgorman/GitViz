...
namespace GitViz.Logic.Tests.LogTests
{
    [TestFixture]
    public class LogRetrieverTests
    {
        ...
        public void ShouldReturnSingleRecentCommitWithLocalRefs()
        {
            using (var tempFolder = new TemporaryFolder())
            {
                ...
                var commit = log.Single();
                var containsHead = commit.Refs[0].Contains("HEAD", StringComparison.OrdinalIgnoreCase);
                Assert.IsTrue(containsHead, "HEAD not found on new repository first commit");
            }
        }
